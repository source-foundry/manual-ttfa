...
- *This application requires the X11 Window system.  See the [freetype2-demos README](https://git.savannah.gnu.org/cgit/freetype/freetype2-demos.git/tree/README) for details on how to install this dependency. macOS users please note that you must install XQuartz **before** you compile the freetype2-demos tools or you will receive a `could not allocate display surface` error message when you attempt to use `ftgrid` (see issue [#5](https://github.com/source-foundry/manual-ttfa/issues/5)) .*
